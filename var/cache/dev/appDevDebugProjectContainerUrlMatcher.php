...
class appDevDebugProjectContainerUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($rawPathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/test')) {
            // testUserReview
            if ('/testUserReview' === $pathinfo) {
                $ret = array (  '_controller' => 'AppBundle\\Controller\\DefaultController::testUserReview',  '_route' => 'testUserReview',);
                if (!in_array($canonicalMethod, array('GET', 'POST'))) {
                    $allow = array_merge($allow, array('GET', 'POST'));
                    goto not_testUserReview;
                }
            ...
            // testtest1
            if (preg_match('#^/test/(?P<name>[^/]++)$#sD', $pathinfo, $matches)) {
                $ret = $this->mergeDefaults(array_replace($matches, array('_route' => 'testtest1')), array (  '_controller' => 'AppBundle\\Controller\\TestController::test',));
                if (!in_array($canonicalMethod, array('GET', 'POST'))) {
                    $allow = array_merge($allow, array('GET', 'POST'));
                    goto not_testtest1;
                }
